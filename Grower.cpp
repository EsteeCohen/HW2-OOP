...
FlowersBouquet* Grower::prepareOrder(std::vector<std::string> flowers){
    std::cout << this->getName() << " forwards the request to " << gardener->getName() << "." << std::endl; //LINE 4
    FlowersBouquet* bouquet = gardener->prepareBouquet(flowers);
    std::cout << gardener->getName() << " returns flowers to " <<  this->getName() << "." << std::endl; //LINE 6
}
